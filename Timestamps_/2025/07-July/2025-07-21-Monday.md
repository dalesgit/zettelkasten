...
---
# 📝 Notes
- I am abandoning the fancy filing of journal entries and returning to the usual structure. The code from format is: 
	- YYYY/MM-MMMM/YYYY-MM-DD-dddd
	- new location: "journal"
Accessed in last month?
```dataview
TABLE WITHOUT ID any(map(date, (d) => d < date(today))), filter(date, (d) => d < date(today)), any(earlierDates) FLATTEN array(map(date, (d) => d < date(today))) as earlierDates
```
---
### Notes created today
```dataview
